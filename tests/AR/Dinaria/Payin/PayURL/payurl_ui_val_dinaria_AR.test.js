...
describe(`[PayURL Dinaria AR] Formularios UI Validations Checkout [Ambiente: ${envConfig.currentEnvName.toUpperCase()}]`, () => {
    ...
    const executeAndGetCheckoutUrl = async (omitField = null) => {
        const payload = {
            ...
            "currency": "ARS",
            "amount": 100.00,
            "merchant_transaction_reference": `PayUrlUI-${Date.now()}`,
            "merchant_customer_id": "cliente_ui_ar@ejemplo.com",
        ...
        if (omitField) {
            delete payload.predefined_fields[0].fields[omitField];
        }
    ...
    const executeFieldValidation = async (testName, omitField, targetSelector, valToType, isHappyOrFinalPath = false) => {
        const url = await executeAndGetCheckoutUrl(omitField);
    ...
    describe('1. Frontend Validations: First Name', () => {
        ...
        test('1.2. First Name: Boundary Largo (51 Chars) (TC12)', async () => {
            ...
            if (r.errorText) {
                expect(r.errorText).toMatch(/Ingresa tu nombre|required|format|length/i);
            }
        ...
        test('1.5. First Name: Happy Path con Caracteres Complejos (Latino/Apostrofes)', async () => {
            ...
            // Comprobación de Voucher Exitoso:
    ...
    describe('2. Frontend Validations: Last Name', () => {
        ...
        test('2.2. Last Name: Boundary Largo (51 Chars) (TC20)', async () => {
            ...
            if (r.errorText) {
                expect(r.errorText).toMatch(/Ingresa tu apellido|required|length/i);
            }
    ...
    describe('3. Frontend Validations: Document Number (CUIL)', () => {
        ...
        test('3.1. CUIL: Prefix Inválido Inexistente (19...)', async () => {
            ...
            expect(r.errorText).toMatch(/Ingresa el CUIL|format/i);
        });
        ...
        test('3.5. CUIL: Módulo 11 Roto (Error ASINC en Backend)', async () => {
            // Un cuil con formato perfectamente correcto pero matemáticamente falso (Dígito=9 en vez de 2).
            // La Regex del Frontend lo deja pasar (Submit Habilitado), pero revienta en la validación Server y tira error en pantalla final.
