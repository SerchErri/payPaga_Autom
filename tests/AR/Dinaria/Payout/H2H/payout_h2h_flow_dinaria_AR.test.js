...
describe(`[E2E Híbrido] Payout H2H Ecuador: API Generación + UI Validaciones [Ambiente: ${envConfig.currentEnvName.toUpperCase()}]`, () => {
    ...
    let page;
    let initialBalances = {};
    let payoutAmount = 150.23; // Fijo para aserciones matemáticas
    ...
    const originarPayoutH2H = async (monto, refTag) => {
        ...
        const payload = {
            ...
            merchant_transaction_reference: `H2H-${refTag}-${Date.now()}`,
            merchant_order_reference: `H2H-OR-${Date.now()}`,
            merchant_customer_id: 'customer@test.com',
            amount: monto,
            fields: {
                first_name: 'Sergio', last_name: 'Errigo', document_type: 'CI',
                document_number: '1710034065', account_number: '1234567891',
                bank_code: 'banco_pichincha', account_type: 'ahorro'
            }
        };
    ...
    test('1. Flujo Ómnicanal Happy Path: UI Saldos -> API Payout H2H -> Visto en M-Portal -> Admin Approve', async () => {
        ...
        // ==========================================
        await fastAdminAction(page, realGridTxId || generatedTxId, 'pay-out', allure, 'approve');
