...
describe(`[E2E Híbrido] V1 Crear Link de Pago (PayUrl) EC [Amb: ${envConfig.currentEnvName.toUpperCase()}]`, () => {
    ...
    test('Flujo Ómnicanal: Capturar Saldo UI -> Generar PayUrl API -> Inspeccionar Grilla -> Aprobar Admin', async () => {
        ...
        // ============================================================================== //
        const myRefId = `PayUrl-V1-EC-${Date.now()}`;
        const payUrlEndpoint = `${envConfig.BASE_URL}/v1/payurl`;
        const validPayload = {
            "country_code": "EC",
            "currency": "USD",
            ...
            "payment_method_data": [
                {
                    ...
                    "transaction_fields": [
                        ...
                        { "name": "email", "value": "serrigo@paypaga.com" },
                        { "name": "document_type", "value": "CI" },
                        { "name": "document_number", "value": "1307990091" }
                    ]
                }
            ]
        };
