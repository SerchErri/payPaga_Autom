...
describe(`[EC] [DoPayment] [Payout] [MerchantPortal] [DEV] Validation Suite`, () => {
    ...
    const attemptSubmit = async (page) => {
        if (allure && allure.attachment) {
            try {
                ...
                allure.attachment("📸 Formulario Lleno (Antes de Enviar)", await page.screenshot({ fullPage: true }), "image/png");
            } catch(e) {}
        }
    ...
    const attemptHappySubmit = async (page) => {
        if (allure && allure.attachment) {
            try {
                await page.waitForTimeout(500); 
                allure.attachment("📸 Formulario Lleno (Antes de Enviar - OK)", await page.screenshot({ fullPage: true }), "image/png");
            } catch(e) {}
        }
        const btn = page.getByRole('button', { name: 'Crear Pago' }).first();
        await btn.click();
    ...
    describe('3. Suite Payout UI: Tipo y Numero de Documento', () => {
        ...
        test('3.4. PP: Falla Desborde (14 Digitos)', async () => {
            ...
            await attemptSubmit(sharedPage); 
            const r = await attachEvidence('PP 14 Digitos', sharedPage, "Documento: '14'");
            expect(r.isBotonBloqueado || r.errorVisualExtraido.length > 0).toBe(true);  
        });
    });
    ...
    describe('4. Suite Payout UI: Comportamiento Monetario (Amount)', () => {
        ...
        test('4.3. Amount: Exceso Decimal (3)', async () => {
            await fillBaseForm(sharedPage);
    ...
    describe('6. Suite UI: Flujo Feliz de Creación y Reflejo en Grilla', () => {
        test('6.1 Creación Exitosa y Búsqueda en Módulo de Transacciones Payout', async () => {
            await fillBaseForm(sharedPage);
            
            // Promise para interceptar y obtener el id del backend al crear
            const responsePromise = sharedPage.waitForResponse(r => r.url().includes('payouts') && (r.status() === 201 || r.status()===200), { timeout: 15000 }).catch(()=>null);
            
            const btnSave = sharedPage.getByRole('button', { name: 'Crear Pago' }).first();
            await btnSave.click();
            ...
            if(res){
                ...
                capturedRef = body.merchant_transaction_reference || body.id || `TX-${Date.now()}`;
            }

            await sharedPage.waitForTimeout(3000); 

            // Navegación a grilla payout instruida
            await sharedPage.goto("https://merchant.v2.dev.paypaga.com/transactions/pay-out", { waitUntil: 'domcontentloaded' });
            ...
            if (allure && allure.attachment) {
                ...
                allure.attachment("📸 Reflejo en Grilla Payout", buffer, "image/png");
            }
